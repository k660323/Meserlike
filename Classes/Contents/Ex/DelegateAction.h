...
public:
    ...
    // 콜백 제거 (콜백 함수 자체로 제거)
    void removeCallback(std::function<void(Args...)> callbackToRemove) {
        // 콜백 리스트에서 제거할 콜백을 찾아 제거
        callbacks.erase(
            std::remove_if(callbacks.begin(), callbacks.end(),
                [&callbackToRemove](const std::function<void(Args...)>& callback) {
    ...
    //// 인자가 없는 경우의 invoke
    template <typename = typename std::enable_if<(sizeof...(Args) == 0)>::type>
    void invoke() {
        for (auto& callback : callbacks) {
            if (callback) {
...
private:
    std::vector<std::function<void(Args...)>> callbacks;  // 콜백 리스트
};